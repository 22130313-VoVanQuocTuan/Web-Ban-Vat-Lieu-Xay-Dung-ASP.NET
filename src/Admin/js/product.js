...
async function fetchProducts(page = 1) {
  try {
    const response = await fetch(
      `http://localhost:5241/api/Product?page=${page}&size=${pageSize}`,
      {
        method: "GET",
        headers: {
...
function displayProducts() {
  ...
  products.forEach((product) => {
    ...
    const imageUrl = product.urlImage.startsWith("http")
      ? product.urlImage
      : `http://localhost:5241${product.urlImage}`;
    productRow.innerHTML = `
            ...
            <td class="bt">
                 <button class="view-description-button" data-product-id="${
                   product.productId
                 }">Mô tả</button>
...
async function getDescription(event) {
  ...
  const response = await fetch(
    `http://localhost:5241/api/Product/description/${productId}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
  ...
  if (data.status === 200) {
    ...
    const decodedDescription = parser.parseFromString(description, "text/html")
      .body.innerHTML;

    // Hiển thị mô tả vào modal
    document.getElementById("productDescriptionDetails").innerHTML =
      decodedDescription;
...
async function addProduct() {
  ...
  const productStatusText =
    productStatusElement.options[productStatusElement.selectedIndex]
      .textContent;
  ...
  formData.append(
    "Description",
    CKEDITOR.instances.productDescriptions.getData()
  );
...
async function deleteProduct(event) {
  ...
  try {
    const response = await customFetch(
      `http://localhost:5241/api/Product/${productId}`,
      {
        method: "DELETE",
    ...
    CKEDITOR.replace("updateProductDescription", {
      on: {
        instanceReady: function () {
          CKEDITOR.instances.updateProductDescription.setData(
            product.description
          );
        },
      },
...
async function saveProductChanges(event) {
  ...
  formData.append(
    "price",
    parseFloat(document.getElementById("updateProductPrice").value)
  );
  formData.append(
    "categoryId",
  ...
  formData.append(
    "stockQuantity",
    parseInt(document.getElementById("updateProductStock").value)
  );
  formData.append(
    "description",
    CKEDITOR.instances.updateProductDescription.getData()
  );
  formData.append(
    "status",
    document.getElementById("updateProductStatus").value
  );
...
document
  .getElementById("saveChangesButton")
  .addEventListener("click", saveProductChanges);
